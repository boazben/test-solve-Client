...
export default function Row({test}) {
    ...
    return (
        ...
        <tr className={showInfo ? Style.bold : null}>
            ...
            <td className="webTable"> {(new Date(test.created)).toLocaleDateString('en-GB')}</td>
            <td className="webTable"> {test.deadline ? (new Date(test.deadline)).toLocaleDateString('en-GB') : "ללא דד-ליין"}</td>
            <td className="webTable">
                {
